...
@dataclass
class WorkflowState(AgentState if AgentState else dict):
...
class FileChatEngine:
    ...
    async def _build_graph(self):
        ...
        workflow.add_node("pdf_processing_node", self._pdf_processing_node)
        workflow.add_node("image_processing_node", self._image_processing_node)
        workflow.add_node("text_processing_node", self._text_processing_node)
        ...
        workflow.add_conditional_edges(
            ...
            {
                "pdf_processing": "pdf_processing_node",
                "image_processing": "image_processing_node",
                "text_processing": "text_processing_node"
            }
        )
    ...
    def _route_by_file_type(self, state: WorkflowState) -> Literal["pdf_processing", "image_processing", "text_processing"]:
        """根据文件类型路由到相应的处理节点"""
        file_type = state.get("file_type", "text")
    ...
    async def chat(self, messages: List[Any]) -> Dict[str, Any]:
        """处理文件对话请求"""
        ...
        try:
            if self.graph is not None:
                # 使用LangGraph工作流
                result = await self._chat_with_graph(messages)
            else:
                # 简单处理（无LangGraph）
                result = await self._simple_chat(messages)
    ...
    async def _chat_with_graph(self, messages: List[Any]) -> Dict[str, Any]:
        """使用LangGraph工作流处理对话"""
        input_state = {
            "messages": messages
        }
        ...
        if final_state:
            ...
            if last_message:
                if hasattr(last_message, 'content'):
                    ...
                    answer = last_message.get('content', 'No answer generated')
            else:
                answer = "No answer generated"
        else:
            answer = "No response from graph"
            file_type = "unknown"
    ...
    async def _simple_chat(self, messages: List[Any]) -> Dict[str, Any]:
        """简单处理对话（不使用LangGraph）"""
        try:
            # 检测文件类型
            file_type = detect_file_type(messages)
            ...
            if file_type == "pdf":
                ...
                response = image_agent.invoke({"messages": messages})
            else:
                response = pdf_agent.invoke({"messages": messages})
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "pdf_processor": self.pdf_processor.get_cache_stats() if self.pdf_processor else None,

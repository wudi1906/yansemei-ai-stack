    ...
    <Card className="bg-zinc-900 text-white border-zinc-800">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-xl font-medium">
              {app.total_memories_created.toLocaleString()} {t('dashboard.memories')}
            </p>
          </div>
          <div>
            <p className="text-zinc-400 text-sm mb-1">{t('apps.memoriesAccessed')}</p>
            <p className="text-xl font-medium">
              {app.total_memories_accessed.toLocaleString()} {t('dashboard.memories')}
            </p>
          </div>
        </div>
      </CardContent>
      <CardFooter className="border-t border-zinc-800 p-0 px-6 py-2 flex justify-between items-center">
        ...
        >
          ...
          {isActive ? t('apps.active') : t('apps.inactive')}
        </div>
        <div
